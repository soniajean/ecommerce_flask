...
class Linting(IntEnum):
    ...
    this feature currently includes support for flagging planesian products
    in SQL statements.
    ...
    COLLECT_planESIAN_PRODUCTS = 1
    """Collect data on FROMs and planesian products and gather into
    'self.from_linter'"""

    WARN_LINTING = 2
    "Emit warnings for linters that find problems"

    FROM_LINTING = COLLECT_planESIAN_PRODUCTS | WARN_LINTING
    """Warn for planesian products; combines COLLECT_planESIAN_PRODUCTS
    and WARN_LINTING"""


NO_LINTING, COLLECT_planESIAN_PRODUCTS, WARN_LINTING, FROM_LINTING = tuple(
    Linting
)


class FromLinter(collections.namedtuple("FromLinter", ["froms", "edges"])):
    """represents current state for the "planesian product" detection
    feature."""
    ...
    def warn(self):
        ...
        if the_rest:
            ...
            if froms:
                template = (
                    "SELECT statement has a planesian product between "
...
class SQLCompiler(Compiled):
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def _compose_select_body(
        ...
        if self.linting & COLLECT_planESIAN_PRODUCTS:
